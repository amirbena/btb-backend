...
const buildResponseValidation = (errors: ValidationError[]): string =>{
    const validationErrors = errors.map((err) => ({
        field: err.property,
        message: Object.values(err.constraints)[0],
    }));
    return validationErrors.map(validationError => validationError.message).join('\n');
} 
...
export const isValidRegister = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        console.log(errors);
        if (errors.length > 0) {
            return res.status(HttpStatusCode.BadRequest).json({ message: buildResponseValidation(errors) });
        }
        next();
    } catch (error) {
